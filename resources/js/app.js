...
const warningDialog = (message, title = 'Periksa kembali') => {
    ...
    return dialog.fire({
        ...
        focusConfirm: true,
        confirmButtonText: '<i class="bi bi-check-lg" aria-hidden="true"></i> Mengerti',
        customClass: dialogClasses(confirmButtonClasses.warning),
        didOpen: (popup) => {
            window.setTimeout(() => popup.querySelector('.swal2-confirm')?.focus(), 50);
        },
    }).then((result) => {
...
document.addEventListener('submit', async (event) => {
    ...
    const confirmationSource = submitter?.matches?.('[data-sim-confirm]')
        ? submitter
        : form.matches('[data-sim-confirm]') ? form : null;
    ...
    const result = await dialog.fire({
        ...
        title: confirmationSource.dataset.simConfirmTitle ?? 'Lanjutkan tindakan ini?',
        text: confirmationSource.dataset.simConfirmText ?? '',
        confirmButtonText: confirmationSource.dataset.simConfirmButton ?? 'Ya, lanjutkan',
        cancelButtonText: 'Batal',
        showCancelButton: true,
        focusCancel: tone === 'danger',
        customClass: dialogClasses(confirmButtonClasses[tone] ?? confirmButtonClasses.primary),
    });
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.app-body .table-responsive > table.table').forEach((table) => {
    ...
    document.querySelectorAll('[data-primary-url], [data-primary-document-trigger]').forEach((record) => {
        ...
        record.addEventListener('keydown', (event) => {
            if ((event.key === 'Enter' || event.key === ' ') && event.target === record) {
                event.preventDefault();
                openPrimaryRecord(record);
            }
        });
